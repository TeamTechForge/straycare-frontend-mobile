...
const logRequest = (url: string, token: string | null, method: string = "GET") => {
  ...
  console.log(`[chatApi Debug] 🌐 Request URL: [${method}] ${url}`);
  console.log(`[chatApi Debug] 🔑 Token Exists: ${tokenExists}, Token Length: ${tokenLength}`);
};

const logResponse = (url: string, status: number) => {
  console.log(`[chatApi Debug] 📥 Response Status for [${url}]: ${status}`);
};
