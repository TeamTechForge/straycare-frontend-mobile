...
export default function VetProfile() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error("Fetch Vet profile error:", error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const stats = [
    { value: profile?.animalsTreated || 0, label: "TREATS" },
  ...
  const userData = {
    name: user?.name || "Veterinarian",
    location: profile?.clinicAddress || profile?.primaryLocation || "Not set",
        ...
        <View style={styles.sectionContent}>
          {activeTab === "posts" && (
            <EmptyStateCard
              icon="paw"
              title="No posts yet."
              subtitle="Share your medical cases and animal care tips."
            />
          )}
          ...
          {activeTab === "rescues" && (
            rescues.length > 0 ? (
              ...
              <EmptyStateCard
                ...
                subtitle="Current treatments and medical cases will appear here."
              />
            )
          )}
